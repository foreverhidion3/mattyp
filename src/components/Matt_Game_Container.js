...
function Game_Container() {
    ...
    const handleHealth = () => {
        sethitsCounted((prevCount) => Math.max(0, prevCount - 200)); // Ensure health points never go below 0
    };
    useEffect(() => {
   ...
   useEffect(() => {
    const handleBlackHoleDamage = () => {
        ...
        if (distanceToBottom <= 200) {
            ...
            sethitsCounted((prevCount) => Math.max(0, prevCount + .2)); // Reduce health by 50 points
        }
    };
